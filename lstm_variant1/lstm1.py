...
prefix_st = matched_obj.groups()[0]
dataframe = read_csv(file_st, usecols=[1], engine='python', skipfooter=0)
...
def predict_from_saved_model(trainX1):
	global prefix_st
	with open('model_architecture_'+prefix_st+'.json', 'r') as f:
		model = model_from_json(f.read())
	model.load_weights('model_weights_'+prefix_st+'.h5')
...
for i in range(nop):
	...
	### make predictions
	model.save('model_weights_'+prefix_st+'.h5')
